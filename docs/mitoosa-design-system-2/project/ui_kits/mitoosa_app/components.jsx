...
// ── Brand mark — three-node constellation glyph ──────────────────────────────
function BrandMark({ size=22 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden>
      ...
      <path d="M5.5 18.5 L12 6 L18.5 18.5" fill="none"
        stroke="url(#bm-grad)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" opacity=".55"/>
      <circle cx="5.5" cy="18.5" r="2.4" fill="url(#bm-grad)" />
      <circle cx="18.5" cy="18.5" r="2.4" fill="url(#bm-grad)" />
      <circle cx="12" cy="6" r="3" fill="#fff"
        style={{ filter:'drop-shadow(0 0 4px rgba(96,165,250,.8))' }}/>
    </svg>
  );
}
...
function AppHeader({ credits, onAvatar }) {
  return (
    <div style={{ position:'sticky', top:0, zIndex:5, padding:'12px 16px 10px',
      display:'flex', alignItems:'center', justifyContent:'space-between',
      background:'linear-gradient(180deg, rgba(10,13,23,.85) 0%, rgba(10,13,23,.55) 70%, transparent 100%)',
      backdropFilter:'blur(8px)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, minWidth:0 }}>
        <button onClick={onAvatar} style={{ width:38, height:38, padding:0, borderRadius:'50%', border:'1.5px solid transparent',
          background:'linear-gradient(var(--bg-1),var(--bg-1)) padding-box, var(--grad-primary) border-box',
          cursor:'pointer', overflow:'hidden', boxShadow:'0 0 14px rgba(96,165,250,.45)', flexShrink:0 }}>
          <img src="../../assets/avatars/avatar_4.png" alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
        </button>
        <div style={{ display:'flex', flexDirection:'column', lineHeight:1, minWidth:0 }}>
          <span style={{ fontSize:17, fontWeight:700, letterSpacing:'-0.035em', color:'#fff' }}>
            mitoosa
          </span>
          <span style={{ fontSize:9, letterSpacing:'0.14em', color:'var(--fg-muted)', textTransform:'uppercase', marginTop:3, fontWeight:600, whiteSpace:'nowrap' }}>
            Pilot · Lv 1
          </span>
        </div>
      </div>
