...
> Spec: [docs/archived/spec-engagement-loop-v1.md](spec-engagement-loop-v1.md)
...
- All 12 tasks in this plan are implemented in the current codebase.
- Verification is complete: `flutter test` passes (258/258) and `flutter analyze` reports 0 issues.
- The checklist items below are kept in sync with verified state during docs closeout; `/test` and `/review` may add, uncheck, or re-check items when they surface drift.
- The final implementation also includes the share-refuel extension (`share_plus` plus `lastShareDate`), even though Task 11 is still written with its original approval-gate wording.
- This plan is archived after ship and kept only as historical execution context.
...
- [x] `computeStars(0)` → 5, `computeStars(1)` → 4, `computeStars(2)` → 3, `computeStars(3)` → 2, `computeStars(4)` → 2, `computeStars(5)` → 1, `computeStars(6)` → 1, `computeStars(7)` → 0
- [x] `computeMasteryTier`: gold ≥ 4.0, silver ≥ 2.5, bronze < 2.5
- [x] `computeAdaptiveMultiplier`: increase at avg ≥ 4.0, decrease at avg < 2.0
- [x] Migration: loading `adaptiveVersion == 1` history [1,2,3] → scaled to [2,3,5] (rounded), `adaptiveVersion` set to 2
- [x] All existing progression engine tests updated and passing

**Verification:**
- [x] `flutter test test/core/engine/progression_engine_test.dart`
...
**Description:** Add 4 new Hive fields to `PlayerProgress` and update `PlayerProgressAdapter`. Fields: `hearts` (int, field 12, default 5), `diamonds` (int, field 13, default 0), `heartRefuelAt` (DateTime?, field 14), `seenTutorialWorlds` (List\<String\>, field 15). The final implementation also adds `lastShareDate` (DateTime?, field 16) from the share-refuel path, so the adapter writes 17 fields total. Add null-safe defaults in the `read()` path for backwards compatibility.

**Acceptance criteria:**

- [x] New `PlayerProgress` objects default to `hearts = 5`, `diamonds = 0`, `heartRefuelAt = null`, `seenTutorialWorlds = []`
- [x] Old saves missing fields 12–16 load correctly with defaults (no crash)
- [x] `write()` serialises all 17 fields in correct order
- [x] `adaptiveVersion == 1` histories are migrated to 0–5 scale in the `read()` path (per Task 1 logic)

**Verification:**
- [x] `flutter test test/data/player_progress_test.dart`
...
- [x] `GameplayState.hintUsed` defaults to false; once set via `copyWith`, persists in that state object
- [x] Run fields serialise/copy correctly through `copyWith`
- [x] Existing `GameplayState` construction is backward compatible (no required parameters added)
...
- [x] `shouldRefuelByTime(30 minutes ago, now)` → true
- [x] `shouldRefuelByTime(10 minutes ago, now)` → false
- [x] `shouldRefuelByTime(null, now)` → false
- [x] `computeNextRefuelTime(now)` returns `now + 30 minutes`
- [x] `isLowerLevel(2, 5)` → true; `isLowerLevel(5, 2)` → false; `isLowerLevel(3, 3)` → false
...
**Acceptance criteria:**
- [x] `useHint()` with `hearts > 0` → `hintUsed = true`, `hearts` decremented by 1
- [x] `useHint()` with `hearts == 0` → state unchanged, returns false
- [x] `computeStars` result is capped at 3 when `hintUsed == true`
- [x] `startRun(10)` sets `isRunActive = true`, `levelsInRun = 10`, `runTimeRemaining = 10 min`
- [x] `tickTimer` decrements `runTimeRemaining`; reaching zero sets `isRunActive = false`
- [x] `completeLevel()` increments `levelsCompleted`; when `levelsCompleted == levelsInRun` and `isRunActive == true` → emits run bonus

**Verification:**
- [x] `flutter test test/features/gameplay/gameplay_view_model_test.dart`
...
**Acceptance criteria:**
- [x] `deductHeart` reduces hearts by 1; floors at 0
- [x] `refuelHeartsWithDiamond` requires `diamonds >= 1`; sets hearts to 5, decrements diamonds by 1
- [x] `checkAndRefuelHeart` adds 1 heart if `shouldRefuelByTime()` passes; updates `heartRefuelAt`; does nothing if hearts already at 5
- [x] `markTutorialSeen` adds worldId to `seenTutorialWorlds` if not already present
- [x] All methods honour max hearts = 5
...
**Acceptance criteria:**
- [x] 5 hearts renders 5 filled hearts
- [x] 2 hearts renders 2 filled + 3 empty hearts
- [x] Diamond badge shows correct count
- [x] "Refuel with 💎" button visible only when `hearts < 5 && diamonds >= 1`
- [x] Widget renders without overflow on 320px-wide screen

**Verification:**
- [x] `flutter test` (widget test for HeartsBar)
- [x] Manual: open WorldMapScreen, inspect hearts display
...
**Acceptance criteria:**
- [x] Button disabled when `level.hint == null`
- [x] Button shows red heart badge with count when `hearts > 0`
- [x] Button tap with `hearts > 0` reveals hint in bottom sheet and deducts heart
- [x] Button tap with `hearts == 0` shows SnackBar "Not enough hearts – use a 💎 or play a lower level"
- [x] Hint badge is `hintUsed == true` once per level (button becomes "used" state, cannot tap twice)

**Verification:**
- [x] `flutter test`
- [x] Manual: tap hint with/without hearts
...
**Acceptance criteria:**
- [x] Progress bar visible and animated when `isRunActive == true`
- [x] Bar colour shifts green → yellow → red as time decreases
- [x] "Time's up!" banner appears when `runTimeRemaining` reaches zero
- [x] Run Bonus overlay appears when `levelsCompleted == levelsInRun` with timer still active
- [x] Timer pauses when app is backgrounded

**Verification:**
- [x] `flutter test` (timer mock test)
- [x] Manual: start a run, watch countdown
...
**Acceptance criteria:**
- [x] Modal appears on first tap into a world; does not appear on subsequent taps into same world
- [x] Dismissing modal stores worldId in `seenTutorialWorlds` and navigates to gameplay
- [x] "?" button on GameplayScreen re-opens modal (no heart cost)
- [x] Modal content shows world name, icon, subtitle/rule, and CTA

**Verification:**
- [x] Manual: enter new world → modal shows; back + re-enter → modal skipped
- [x] `flutter test` (widget test: modal renders; WorldMapScreen integration)
...
- [x] Open world → tutorial modal appears
- [x] Tap hint → heart deducted, hint shown
- [x] Run timer starts and counts down
- [x] Hearts bar and diamond badge render correctly in WorldMapScreen
...
**Acceptance criteria:**
- [x] `shareAndRefuel` calls native share sheet
- [x] Heart +1 granted on share (if under daily limit)
- [x] Second share same day grants no heart (idempotent)
- [x] No crash on web (share_plus web fallback)

**Verification:**
- [x] `flutter test`
- [x] Manual: tap share on a device
...
**Acceptance criteria:**
- [x] Stars correctly reflect hint cap (max 3 when hint used)
- [x] Run Bonus grants +1 diamond on full-run completion
- [x] Completing level index N when player was blocked at index M > N grants +1 heart
- [x] All star display widgets show up to 5 stars

**Verification:**
- [x] `flutter test`
- [x] Manual full-run from start to finish with/without hints
...
- [x] All success criteria in spec checked off
- [x] share_plus task (11) either complete or explicitly deferred to v2
