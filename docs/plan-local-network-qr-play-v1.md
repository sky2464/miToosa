...
2. [ ] Create `lib/data/network/network_models.dart`:
...
3. [ ] Create `lib/data/network/network_exceptions.dart`:
   - `ConnectionFailedException`
   - `InvalidQRException`
   - `SessionExpiredException`
   - `NetworkTimeoutException`
4. [ ] Create `lib/core/network/local_network_info.dart`:
...
**Tests**:
- [ ] Unit: `network_models_test.dart` → JSON round-trip serialization
- [ ] Unit: `local_network_info_test.dart` → mock IP detection

**Verification**:
- [ ] `dart analyze` passes
- [ ] `flutter test test/data/network/` passes
...
Tasks:
1. [ ] Create `lib/data/network/local_network_server.dart`:
...
**Tests**:
- [ ] Unit: `local_network_server_test.dart`:
...
**Verification**:
- [ ] `flutter test test/data/network/local_network_server_test.dart` passes
- [ ] No memory leaks (connections properly closed)
- [ ] Server binds to 127.0.0.1 in tests (not exposed)
...
Tasks:
1. [ ] Create `lib/data/network/local_network_client.dart`:
...
**Verification**:
- [ ] `flutter test test/data/network/local_network_client_test.dart` passes
- [ ] Works with `local_network_server_test.dart` server
- [ ] Reconnection backoff works correctly
...
Tasks:
1. [ ] Create `lib/data/local_session_provider.dart`:
...
2. [ ] Wire to `localNetworkServer` and `localNetworkClient`:
...
**Tests**:
- [ ] Integration: `local_session_provider_test.dart`:
...
**Verification**:
- [ ] `flutter test test/data/local_session_provider_test.dart` passes
- [ ] Riverpod DevTools shows state transitions
...
Tasks:
1. [ ] Create `lib/features/local_play/qr_host_screen.dart`:
...
**Tests**:
- [ ] Widget: `qr_host_screen_test.dart`:
...
**Verification**:
- [ ] Visual: QR code is crisp and scannable
- [ ] Layout responsive on various phone sizes
...
Tasks:
1. [ ] Create `lib/features/local_play/qr_scanner_screen.dart`:
...
**Verification**:
- [ ] Camera permission flows correctly
...
Tasks:
1. [ ] Create `lib/features/local_play/local_session_screen.dart`:
...
**Tests**:
- [ ] Widget: `local_session_screen_test.dart`:
...
**Verification**:
- [ ] Latency test: move appears on guest within 100ms
...
Tasks:
1. [ ] Create `lib/features/local_play/local_play_router.dart`:
   - Routes:
...
**Tests**:
- [ ] Widget: `local_play_router_test.dart`:
...
- [ ] Unit tests pass with >85% coverage (or 100% for core logic)
- [ ] `dart analyze` shows no issues
- [ ] `flutter test` passes all tests
- [ ] Code is reviewed for security (no hardcoded IPs, secrets, or unsafe network access)
- [ ] Commit message follows convention: `feat(scope): description` or `test(scope): description`
...
- [ ] Manual testing on real Android + iOS devices succeeds
- [ ] QR scan succeeds in normal lighting within 3 tries
- [ ] Local session completes 10+ consecutive rounds without crash
- [ ] Scores are persisted correctly to Hive
- [ ] No sensitive data leaked in logs or network traffic
