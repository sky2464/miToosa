...
- [ ] `pubspec.yaml` has `firebase_core` and `firebase_analytics` (versions verified on pub.dev)
- [ ] `FirebaseAnalyticsSink` implemented in `lib/core/analytics/firebase_analytics_sink.dart`
- [ ] `analyticsSinkProvider` wired with `kFirebaseEnabled` compile-time flag
- [ ] `lib/firebase_options.dart` placeholder exists with compile-error guard
- [ ] `lib/main.dart` includes conditional `Firebase.initializeApp()`
- [ ] `docs/ANALYTICS-SETUP.md` written with all manual steps
- [ ] Default build (`FIREBASE_ENABLED=false`): `dart analyze` passes, `flutter test` 637+ pass
- [ ] Default build behaviour identical to pre-change (NoOpAnalyticsSink active)
...
- [ ] **1. Dependency and bootstrap wiring**
  - [ ] 1.1 Verify latest stable `firebase_core` and `firebase_analytics` versions and add to `pubspec.yaml` — _Requirements: AC-002_
  - [ ] 1.2 Add guarded `Firebase.initializeApp()` path in `lib/main.dart` using `FIREBASE_ENABLED` flag — _Requirements: AC-001, AC-005_
- [ ] **2. Analytics sink integration**
  - [ ] 2.1 Implement `FirebaseAnalyticsSink` adapter in `lib/core/analytics/firebase_analytics_sink.dart` — _Requirements: AC-002, AC-003_
  - [ ] 2.2 Wire `analyticsSinkProvider` to switch between `NoOpAnalyticsSink` and Firebase sink by flag in `lib/data/telemetry_provider.dart` — _Requirements: AC-001, AC-003_
- [ ] **3. Safety guards and docs**
  - [ ] 3.1 Add placeholder `lib/firebase_options.dart` compile guard for enabled-without-config scenario — _Requirements: AC-004_
  - [ ] 3.2 Create `docs/ANALYTICS-SETUP.md` with FlutterFire and DebugView manual setup flow — _Requirements: AC-006_
- [ ] **4. Verification**
  - [ ] 4.1 Run `flutter pub get`, `dart analyze`, and `flutter test` with `FIREBASE_ENABLED=false` baseline — _Requirements: AC-001, AC-007_
