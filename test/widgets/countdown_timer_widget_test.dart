...
void main() {
  group('CountdownTimerWidget', () {
    ...
    testWidgets('text color is NOT pink when remainingSeconds > 4',
      ...
      expect(textWidget.style?.color, isNot(equals(AP.pink)),
    ...
    testWidgets('text color IS pink when remainingSeconds == 0', (tester) async {
      await tester.pumpWidget(_wrap(
    ...
    testWidgets('totalSeconds == 0 does not crash (defensive)',
        (tester) async {
      await tester.pumpWidget(_wrap(
        const CountdownTimerWidget(remainingSeconds: 0, totalSeconds: 0),
      ));
