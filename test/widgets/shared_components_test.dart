...
void main() {
  ...
  group('StatPill', () {
    ...
    testWidgets('glow=true renders without error', (tester) async {
      await tester.pumpWidget(_wrap(
        const StatPill(label: 'Glow', glow: true),
      ));
    ...
    for (final tint in StatPillTint.values) {
      testWidgets('renders ${tint.name} tint without error', (tester) async {
        await tester.pumpWidget(_wrap(
          StatPill(label: tint.name, tint: tint),
        ));
  ...
  group('AppHeader', () {
    testWidgets('renders brand name miToosa', (tester) async {
      await tester.pumpWidget(_wrap(
        const AppHeader(credits: 500),
      ));
      expect(find.text('miToosa'), findsOneWidget);
    });

    testWidgets('renders formatted credits for values under 1000', (tester) async {
      await tester.pumpWidget(_wrap(
        const AppHeader(credits: 500),
      ));
    ...
    testWidgets('renders formatted credits with comma for 1000+', (tester) async {
      await tester.pumpWidget(_wrap(
        const AppHeader(credits: 1250),
      ));
    ...
    testWidgets('renders roleLabel in uppercase', (tester) async {
      await tester.pumpWidget(_wrap(
        const AppHeader(credits: 0, roleLabel: 'Pilot · Lv 3'),
      ));
    ...
    testWidgets('onAvatarTap fires when avatar is tapped', (tester) async {
      var tapped = false;
      await tester.pumpWidget(_wrap(
        AppHeader(credits: 0, onAvatarTap: () => tapped = true),
      ));
  ...
  group('PrimaryButton', () {
    ...
    testWidgets('renders in disabled state when onPressed is null', (tester) async {
      await tester.pumpWidget(_wrap(
        const PrimaryButton(
          onPressed: null,
          child: Text('Locked'),
