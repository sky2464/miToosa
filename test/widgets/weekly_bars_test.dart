  ...
  group('WeeklyBars', () {
    testWidgets('renders all 7 day labels', (tester) async {
      await tester.pumpWidget(
        _wrap(const WeeklyBars(dailyXp: [3, 0, 8, 12, 0, 15, 2])),
      );
    ...
    testWidgets('asserts exactly 7 values', (tester) async {
      await tester.pumpWidget(_wrap(const WeeklyBars(dailyXp: [1, 2, 3])));
      expect(tester.takeException(), isAssertionError);
    });

    testWidgets('custom todayIndex highlights different column', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const WeeklyBars(dailyXp: [5, 5, 5, 5, 5, 5, 5], todayIndex: 2)),
      );
      await tester.pumpAndSettle();
